...
- **447 eval scenarios** · precision = recall = **F1 = 1.000**. CI fails if *any* scenario fails, and separately if F1 drops below 0.95 — an aggregate alone would let a localized regression through. The harness is
...
- **Full-severity precision on expert T-SQL (2026-08-22).** A second, larger held-out corpus — 53 files / 6.6 MB from the First Responder Kit, Ola Hallengren's Maintenance Solution, sp_WhoIsActive, tSQLt, DarlingData and Microsoft's samples (`tools/heldout-corpus-full-severity.md` lists the commits; the per-rule triage table is beside it) — with a hand-triaged sample of *every* rule that fired, at every severity, not just error-and-above. Measured false-positive rate **42 % before the fix wave, 5.4 % after** (461 of 2,435 findings triaged; the remaining findings are mostly "technically true, useless here" noise such as `missing_schema_prefix` on DBA tooling). Everything classified as a false positive is now fixed root-cause and pinned by a scenario; the number is our own classification, so assume the next reviewer finds more.
- **Blind production trial (connected advisor).** `tools/blind-trial/` builds a database with **18 planted defects and 5 benign look-alikes**, runs a workload, and grades dbopt's Health/Advise output against measured ground truth — built and graded by people who had not read each other's work. First run: 11 hits, 2 partial, 6 misses, 5 false alarms. After the fixes: **18 / 18, no false alarm on the look-alikes.** The misses were real gaps (fragmentation, fill factor, forwarded records, stale statistics, parameter-sniffing skew, deprecated LOB columns, a dropped query-level `PlanAffectingConvert`), not tuning.
