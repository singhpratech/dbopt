...
- The static analyzer runs **103 rules** in milliseconds with a self-graded **F1 = 1.000** on our
  304-scenario corpus (covering every rule, plus 13 more for the 12 plan-XML/DMV checks) — so the flags it's tested on are precise, not noise.
...
- **Trust:** live-tested against SQL Server **2025**; 2014–2022 are supported static-analysis targets; static-rule self-graded **F1 = 1.000**
  across 304 tagged scenarios (every rule covered); **never reads your data**; never auto-executes changes.
