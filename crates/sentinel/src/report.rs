...
pub fn render_markdown(r: &WeeklyReport) -> String {
    ...
    let _ = writeln!(s, "# dbopt sentinel · pain report");
    let _ = writeln!(s);
    let _ = writeln!(
        s,
        "**Window** &nbsp; {} → {}  \n**Instances tracked** &nbsp; {}",
    ...
    let _ = writeln!(
        s,
        "---\n_Generated by `dbopt-sentinel`. Each row above can be cross-referenced against the static analyzer findings in the AI workspace._"
    );
    s
}
...
mod tests {
    ...
    fn renders_empty_report() {
        ...
        assert!(md.contains("# dbopt sentinel"));
